...
public sealed class Closure : IPhpCallable, IPhpPrintable
{
    ...
    public PhpValue __invoke(params ReadOnlySpan<PhpValue> parameters)
    {
        if (_callable is PhpAnonymousRoutineInfo)
        {
            ...
            using var newArgsBuffer = MemoryPool<PhpValue>.Shared.Rent(1 + @static.Count + parameters.Length);
            var newargs = newArgsBuffer.Memory.Span.Slice(0, 1 + @static.Count + parameters.Length);
            ...
            parameters.CopyTo(newargs.Slice(1 + @static.Count));
